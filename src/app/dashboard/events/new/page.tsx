      ...
      <div className="flex items-center gap-2">
        {steps.map((s, i) => (
          <div key={s.id} className="flex items-center gap-2 flex-1">
            <div className={cn("w-9 h-9 rounded-full flex items-center justify-center shrink-0", step > s.id ? "bg-emerald-600 text-white" : step === s.id ? "bg-teal-600 text-white" : "bg-slate-800 text-slate-500")}>
              {step > s.id ? <Check className="w-4 h-4" /> : <s.icon className="w-4 h-4" />}
            </div>
            <span className={cn("text-xs hidden md:block", step >= s.id ? "text-slate-700" : "text-slate-600")}>{s.title}</span>
      ...
      <div className="glass-card p-6">
        {step === 1 && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div><label className="text-xs text-slate-500 block mb-1">Event Type *</label><select value={form.type} onChange={(e) => update("type", e.target.value)} className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2.5 text-sm text-slate-700 focus:outline-none focus:border-violet-500/50"><option value="">Select type</option><option>Corporate</option><option>Wedding</option><option>Conference</option><option>Social</option><option>Festival</option></select></div>
              <div><label className="text-xs text-slate-500 block mb-1">Start Date *</label><input type="date" value={form.startDate} onChange={(e) => update("startDate", e.target.value)} className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2.5 text-sm text-slate-800 focus:outline-none focus:border-violet-500/50" /></div>
              <div><label className="text-xs text-slate-500 block mb-1">End Date</label><input type="date" value={form.endDate} onChange={(e) => update("endDate", e.target.value)} className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2.5 text-sm text-slate-800 focus:outline-none focus:border-violet-500/50" /></div>
            </div>
        ...
        {step === 4 && (
          <div className="space-y-4">
            ...
            <div><label className="text-xs text-slate-500 block mb-1">Event Manager</label><select value={form.manager} onChange={(e) => update("manager", e.target.value)} className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2.5 text-sm text-slate-700 focus:outline-none focus:border-violet-500/50"><option value="">Select manager</option><option>John Smith</option><option>Lisa Park</option><option>Miguel Santos</option><option>Rachel Green</option></select></div>
          </div>
        )}
        ...
        <div className="flex items-center justify-between mt-6 pt-4 border-t border-slate-200">
          <button onClick={() => setStep((p) => Math.max(1, p - 1))} disabled={step === 1} className="flex items-center gap-2 px-4 py-2 bg-slate-800 text-slate-700 text-sm rounded-lg disabled:opacity-30"><ArrowLeft className="w-4 h-4" /> Back</button>
          {step < 5 ? (
            <button onClick={() => setStep((p) => Math.min(5, p + 1))} className="flex items-center gap-2 px-4 py-2 bg-teal-600 hover:bg-violet-500 text-white text-sm rounded-lg">Next <ArrowRight className="w-4 h-4" /></button>
          ) : (

    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {showCompose && (
        <div className="glass-card p-6">
          <h3 className="text-base font-semibold text-slate-200 mb-4">Compose Message</h3>
          <div className="flex gap-2 mb-4">
            {["Email", "SMS", "WhatsApp"].map((t) => (
              <button key={t} onClick={() => setComposeType(t)} className={cn("px-3 py-1.5 text-sm rounded-lg transition-colors", composeType === t ? "bg-violet-600 text-white" : "bg-slate-800/50 text-slate-400 hover:text-white")}>{t}</button>
            ))}
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label className="text-xs text-slate-500 block mb-1">Event</label><select className="w-full bg-slate-900/40 border border-slate-800/40 rounded-xl px-3 py-2 text-sm text-slate-300 focus:outline-none focus:border-violet-500/50"><option>Select event</option><option>NexGen Series X Launch</option><option>Summer Music Festival</option><option>Elite Awards Night 2026</option></select></div>
            <div><label className="text-xs text-slate-500 block mb-1">Recipients</label><select className="w-full bg-slate-900/40 border border-slate-800/40 rounded-xl px-3 py-2 text-sm text-slate-300 focus:outline-none focus:border-violet-500/50"><option>All Guests</option><option>VIP Only</option><option>Speakers</option><option>Not Responded</option></select></div>
            <div className="md:col-span-2"><label className="text-xs text-slate-500 block mb-1">Subject</label><input type="text" placeholder="Message subject" className="w-full bg-slate-900/40 border border-slate-800/40 rounded-xl px-3 py-2 text-sm text-slate-200 placeholder-slate-600 focus:outline-none focus:border-violet-500/50" /></div>
            <div className="md:col-span-2"><label className="text-xs text-slate-500 block mb-1">Message</label><textarea rows={4} placeholder="Write your message..." className="w-full bg-slate-900/40 border border-slate-800/40 rounded-xl px-3 py-2 text-sm text-slate-200 placeholder-slate-600 focus:outline-none focus:border-violet-500/50" /></div>
          </div>
          <div className="flex gap-3 mt-4">
            <button className="flex items-center gap-2 px-4 py-2 bg-violet-600 hover:bg-violet-500 text-white text-sm rounded-lg"><Send className="w-4 h-4" /> Send Now</button>
            <button className="px-4 py-2 bg-slate-800 text-slate-300 text-sm rounded-lg">Schedule</button>
      ...
      <div className="flex items-center gap-3">
        <div className="relative max-w-xs w-full"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500" />
          <input type="text" placeholder="Search communications..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full bg-slate-900/40 border border-slate-800/40 rounded-xl pl-10 pr-4 py-2 text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:border-violet-500/50" /></div>
      </div>
      ...
      <div className="glass-card overflow-hidden">
        <table className="w-full"><thead><tr className="border-b border-slate-800/50">
          ...
          {filtered.map((c) => (
            <tr key={c.id} className="border-b border-slate-800/20 table-row-hover">
              ...
              <td className="px-4 py-3 text-sm text-slate-200">{c.subject}</td>
      ...
      <div>
        <h3 className="text-base font-semibold text-slate-200 mb-3">Templates</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {templates.map((t) => (
            <div key={t.id} className="glass-card p-4 cursor-pointer hover:border-violet-500/30 transition-all">
              ...
              <h4 className="text-sm font-semibold text-slate-200 mt-2">{t.name}</h4>

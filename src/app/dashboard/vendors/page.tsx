  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-slate-100">Vendor Directory</h1><p className="text-sm text-slate-500 mt-0.5">Manage your vendor relationships</p></div>
        <button onClick={() => setShowForm(!showForm)} className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-violet-600 to-violet-500 hover:from-violet-500 hover:to-violet-400 text-white text-sm font-semibold rounded-xl transition-all shadow-lg shadow-violet-500/20 hover:shadow-violet-500/30"><Plus className="w-4 h-4" /> Add Vendor</button>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: "Total Vendors", value: vendors.length.toString(), color: "text-violet-400" },
      ...
      <div className="flex items-center gap-3">
        <div className="relative max-w-xs w-full"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500" />
          <input type="text" placeholder="Search vendors..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full bg-slate-900/40 border border-slate-800/40 rounded-xl pl-10 pr-4 py-2 text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:border-violet-500/50" /></div>
      ...
      {showForm && (
        <div className="glass-card p-6">
          <h3 className="text-base font-semibold text-slate-200 mb-4">Add New Vendor</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label className="text-xs text-slate-500 block mb-1">Vendor Name *</label>
              <input type="text" placeholder="Vendor Name" value={vForm.name} onChange={(e) => setVForm({...vForm, name: e.target.value})} className="w-full bg-slate-900/40 border border-slate-800/40 rounded-xl px-3 py-2 text-sm text-slate-200 placeholder-slate-600 focus:outline-none focus:border-violet-500/50" /></div>
            <div><label className="text-xs text-slate-500 block mb-1">Category</label>
              <select value={vForm.category} onChange={(e) => setVForm({...vForm, category: e.target.value})} className="w-full bg-slate-900/40 border border-slate-800/40 rounded-xl px-3 py-2 text-sm text-slate-300 focus:outline-none focus:border-violet-500/50">
            ...
            <div><label className="text-xs text-slate-500 block mb-1">Contact Person</label>
              <input type="text" placeholder="Contact Person" value={vForm.contactPerson} onChange={(e) => setVForm({...vForm, contactPerson: e.target.value})} className="w-full bg-slate-900/40 border border-slate-800/40 rounded-xl px-3 py-2 text-sm text-slate-200 placeholder-slate-600 focus:outline-none focus:border-violet-500/50" /></div>
            <div><label className="text-xs text-slate-500 block mb-1">Email *</label>
              <input type="email" placeholder="Email" value={vForm.email} onChange={(e) => setVForm({...vForm, email: e.target.value})} className="w-full bg-slate-900/40 border border-slate-800/40 rounded-xl px-3 py-2 text-sm text-slate-200 placeholder-slate-600 focus:outline-none focus:border-violet-500/50" /></div>
            <div><label className="text-xs text-slate-500 block mb-1">Phone</label>
              <input type="text" placeholder="Phone" value={vForm.phone} onChange={(e) => setVForm({...vForm, phone: e.target.value})} className="w-full bg-slate-900/40 border border-slate-800/40 rounded-xl px-3 py-2 text-sm text-slate-200 placeholder-slate-600 focus:outline-none focus:border-violet-500/50" /></div>
            <div><label className="text-xs text-slate-500 block mb-1">Website</label>
              <input type="text" placeholder="Website" value={vForm.website} onChange={(e) => setVForm({...vForm, website: e.target.value})} className="w-full bg-slate-900/40 border border-slate-800/40 rounded-xl px-3 py-2 text-sm text-slate-200 placeholder-slate-600 focus:outline-none focus:border-violet-500/50" /></div>
          </div>
          <div className="flex gap-3 mt-4">
            <button onClick={handleAddVendor} className="px-4 py-2 bg-violet-600 hover:bg-violet-500 text-white text-sm rounded-lg flex items-center gap-2"><CheckCircle2 className="w-4 h-4" /> Save Vendor</button>
        ...
        {filtered.map((vendor) => (
          <Link key={vendor.id} href={`/dashboard/vendors/${vendor.id}`} className="glass-card p-5 group cursor-pointer">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-3">
                <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-violet-500/20 to-purple-500/20 flex items-center justify-center text-base font-bold text-violet-400">{vendor.name[0]}</div>
                <div>
                  <h3 className="text-sm font-semibold text-slate-200 group-hover:text-violet-300 transition-colors">{vendor.name}</h3>
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-slate-800/50 text-slate-400">{vendor.category}</span>
                </div>
              </div>
            ...
            <div className="flex items-center justify-between mt-4 pt-3 border-t border-slate-800/30">
              ...
              <div className="flex gap-1">{vendor.tags.map((t) => <span key={t} className="text-[9px] px-1.5 py-0.5 rounded bg-slate-800/50 text-slate-500">{t}</span>)}</div>
            </div>
          </Link>
        ))}
      </div>

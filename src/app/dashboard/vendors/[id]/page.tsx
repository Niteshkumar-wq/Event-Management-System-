  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center gap-3">
        <Link href="/dashboard/vendors" className="p-2 rounded-lg hover:bg-slate-100 transition-colors"><ArrowLeft className="w-5 h-5 text-slate-400" /></Link>
      ...
      <div className="flex gap-1 border-b border-slate-200/50">
        {["overview", "bookings", "contracts"].map((tab) => (
          <button key={tab} onClick={() => setActiveTab(tab)} className={cn("px-4 py-2.5 text-sm font-medium capitalize relative transition-colors", activeTab === tab ? "text-teal-600" : "text-slate-500 hover:text-slate-700")}>
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <div key={item.label} className="flex items-center gap-3">
                  ...
                  <div><p className="text-[10px] text-slate-500 uppercase">{item.label}</p><p className="text-sm text-slate-800">{item.value}</p></div>
                </div>
              ))}
            </div>
            <div className="mt-4 pt-4 border-t border-slate-200">
              <p className="text-[10px] text-slate-500 uppercase mb-2">Tags</p>
              <div className="flex gap-2">{vendor.tags.map((t) => <span key={t} className="text-xs px-2 py-1 rounded-lg bg-slate-100 text-slate-400">{t}</span>)}</div>
            </div>
          </div>

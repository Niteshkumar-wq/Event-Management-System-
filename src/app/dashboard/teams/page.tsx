    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {showForm && (
        <div className="glass-card p-6">
          <h3 className="text-base font-semibold text-slate-800 mb-4">Add Team Member</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {["Full Name", "Email", "Phone"].map((f) => (
              <div key={f}><label className="text-xs text-slate-500 block mb-1">{f}</label>
                <input type="text" placeholder={f} className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-800 placeholder-slate-600 focus:outline-none focus:border-violet-500/50" /></div>
            ))}
      ...
      <div className="flex items-center gap-3">
        <div className="relative max-w-xs w-full"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500" />
          <input type="text" placeholder="Search members..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full bg-white border border-slate-200 rounded-xl pl-10 pr-4 py-2 text-sm text-slate-800 placeholder-slate-500 focus:outline-none focus:border-violet-500/50" /></div>
        <select value={filterRole} onChange={(e) => setFilterRole(e.target.value)} className="bg-white border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-700 focus:outline-none focus:border-violet-500/50">
      ...
      <div className="glass-card overflow-hidden">
        <h3 className="text-base font-semibold text-slate-800 px-5 pt-5 pb-3">Staff Directory</h3>
        <table className="w-full">
          ...
          <tbody>
            {filtered.map((member) => (
              <tr key={member.id} className="border-b border-slate-100 table-row-hover">
      ...
      {/* Event Teams */}
      <div>
        <h3 className="text-base font-semibold text-slate-800 mb-3">Event Teams</h3>
